...
impl<T, S: Storage<T>> Internal<T, S> {
	...
	pub fn forget(&mut self) {
		core::mem::forget(core::mem::take(&mut self.other_children))
	}
	...
	pub fn insert_by_key(
		...
		match binary_search_min(|a, b| cmp(&a.item, b), &self.other_children, &item) {
			Some((i, eq)) => {
				if eq {
					core::mem::swap(&mut item, &mut self.other_children[i].item);
					Ok((i.into(), item))
				} else {
					Err(InsertionError {
						item,
	...
	pub fn push_left(&mut self, item: T, mut child_id: S::Node) {
		core::mem::swap(&mut self.first_child, &mut child_id);
		self.other_children.insert(
			0,
			Branch {
				item,
	...
	pub fn pop_left(&mut self) -> Result<(T, S::Node), WouldUnderflow> {
		if self.item_count() <= UNDERFLOW {
			...
			let child_id = core::mem::replace(&mut self.first_child, first.child);
