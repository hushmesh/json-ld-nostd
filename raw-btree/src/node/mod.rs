...
impl<T, S: Storage<T>> Node<T, S> {
	...
	pub fn visit_from_leaves_mut_with(
		...
		if let Node::Internal(node) = self {
			for c in node.children() {
				let child: &mut Self = unsafe { core::mem::transmute(nodes.get_mut(c)) };
...
pub enum Children<'a, T, S: Storage<T>> {
	...
	Internal(
		Option<S::Node>,
		core::slice::Iter<'a, internal::Branch<T, S>>,
	),
}
...
pub enum ChildrenWithSeparators<'a, T, S: Storage<T>> {
	...
	Internal(
		Option<S::Node>,
		Option<&'a T>,
		core::iter::Peekable<core::slice::Iter<'a, internal::Branch<T, S>>>,
	),
}
