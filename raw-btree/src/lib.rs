	...
	pub fn visit_from_leaves_mut(&mut self, mut f: impl FnMut(S::Node, &mut Node<T, S>)) {
		if let Some(root_id) = self.root {
			let root_node: &mut Node<T, S> =
				unsafe { core::mem::transmute(self.nodes.get_mut(root_id)) };
		...
		match self.addr {
			Some(addr) => unsafe {
				if self.len > 0 {
					self.len -= 1;
					self.addr = self.btree.nodes.next_item_address(addr);
					Some(core::mem::transmute(self.btree.get_mut_at(addr).unwrap()))
				} else {
					None
				}
			},
		...
		match self.addr {
			Some(addr) => unsafe {
				if self.len > 0 {
					self.len -= 1;
					self.addr = self.btree.nodes.next_item_address(addr);
					Some(core::ptr::read(self.btree.get_at(addr).unwrap()))
				} else {
					None
				}
			},
