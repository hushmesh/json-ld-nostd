...
#[cfg(not(feature="std"))]
impl<E: 'static + no_std_io::error::Error> no_std_io::error::Error for Error<E> {
	fn source(&self) -> Option<&(dyn no_std_io::error::Error + 'static)> {
		match self {
			Self::Stream(_, e) => Some(e),
			_ => None,
		}
