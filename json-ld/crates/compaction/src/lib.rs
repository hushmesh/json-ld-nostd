...
pub trait CompactFragment<I, B> {
	fn compact_fragment_full<'a, N, L>(
		&'a self,
		vocabulary: &'a mut N,
		...
		loader: &'a L,
		options: Options,
	) -> Pin<Box<dyn Future<Output = CompactFragmentResult> + 'a>>
	where
	...
	#[inline(always)]
	fn compact_fragment_with<'a, N, L>(
		&'a self,
		vocabulary: &'a mut N,
		active_context: &'a Context<I, B>,
		loader: &'a mut L,
...
pub trait CompactIndexedFragment<I, B> {
	...
	fn compact_indexed_fragment<'a, N, L>(
		&'a self,
		vocabulary: &'a mut N,
		index: Option<&'a str>,
		active_context: &'a Context<I, B>,
...
impl<T: CompactFragment<I, B>, I, B, S> CompactFragment<I, B> for IndexSet<T, S> {
	fn compact_fragment_full<'a, N, L>(
		&'a self,
		vocabulary: &'a mut N,
		active_context: &'a Context<I, B>,
...
impl<T: CompactFragment<I, B>, I, B> CompactFragment<I, B> for Vec<T> {
	...
	where
		...
		L: Loader,
	{
		Box::pin(compact_collection_with(
			vocabulary,
			self.iter(),
			active_context,
			type_scoped_context,
